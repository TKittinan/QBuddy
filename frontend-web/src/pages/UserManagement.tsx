...
import { fetchUsers, addUserAsync, updateUserAsync, deleteUserAsync , clearError } from "../redux/userSlice";
...
{error && !loading && filteredUsers.length === 0 && (
        <div className="bg-red-50 text-red-600 p-4 rounded-xl border border-red-100 text-sm flex items-center gap-2 shadow-sm">
          <Shield size={16} className="shrink-0" />
          <p>พบข้อผิดพลาด: {error} (โปรดตรวจสอบสิทธิ์การเข้าถึงหรือลองล็อกอินใหม่)</p>
        </div>
      )}

      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 relative min-h-[300px]">
        {loading && (
          <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10 rounded-xl">
            ...
            <p className="text-sm text-slate-500 font-medium">กำลังดึงข้อมูลผู้ใช้งาน...</p>
          </div>
        )}
        ...
        {filteredUsers.length > 0 ? (
          ...
          !loading && (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p>ไม่พบข้อมูลผู้ใช้งานในระบบ</p>
            </div>
          )
        )}
      </div>
