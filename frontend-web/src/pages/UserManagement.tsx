...
import { Plus, MoreHorizontal, Edit, Trash2, Mail, Shield, Calendar, CheckCircle2, User as UserIcon } from "lucide-react";
...
export default function UserManagement() {
  ...
  const handleConfirmAdd = () => {
    ...
    setAddName(""); setAddEmail(""); setIsAddPanelOpen(false);
  };

  const handleEditClick = (user: User) => {
    setEditingUser(user); setEditName(user.name); setEditEmail(user.email); setEmailError("");
  };
  ...
  const handleConfirmEdit = () => {
    ...
    dispatch(updateUser({ ...editingUser, name: editName, email: editEmail }));
    setEditingUser(null);
  };
  ...
  const columns: Column<User>[] = [
    { header: "USER INFO", key: "name", render: (user) => (
        <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold uppercase">{user.name.charAt(0)}</div><div><p className="font-medium text-slate-800">{user.name}</p><p className="text-xs text-slate-500">{user.email}</p></div></div>
      )},
        ...
        {editingUser && (
          <>
            <div className="flex flex-col items-center text-center mb-10">
              <div className="w-24 h-24 rounded-3xl bg-indigo-600 flex items-center justify-center text-white font-bold text-4xl shadow-xl shadow-indigo-100 mb-4 uppercase">{editingUser.name.charAt(0)}</div>
